...
async function axpRegisterWebPush(){
 ...
 if(/iPhone|iPad|iPod/i.test(navigator.userAgent)&&!axpIsStandalone())throw Error("On iPhone or iPad, first add AXIOM PULSE to the Home Screen, then open the installed app and enable notifications.");
...
function axpNotificationCard(){
 ...
 if(!supported)return '<section class="card"><h3>Delivery Notifications</h3><p class="muted">This device/browser does not support the AXIOM PULSE push notification service.</p></section>';
 if(granted)return '<section class="card"><h3>Delivery Notifications</h3><div class="status ok">Background push notifications enabled on this device.</div><p class="muted">Reminders can arrive even when AXIOM PULSE is closed.</p><button class="btn secondary" onclick="axpTestPushUI()">SEND TEST NOTIFICATION</button></section>';
 if(ios&&!standalone)return '<section class="card"><h3>Enable Delivery Notifications</h3><p class="muted">On iPhone/iPad, first add AXIOM PULSE to your Home Screen. Then open the Home Screen app and tap Enable Notifications.</p><div class="status warn">Home Screen app required for iPhone/iPad push.</div></section>';
...
async function axpTestPushUI(){try{const x=await sendAXPTestPush();note("Test notification sent","Push service accepted the notification for "+(x.sent??0)+" registered device(s). Close AXIOM PULSE now to verify the background notification.")}catch(e){note("Test notification failed",e.message)}}
